...
from models.RuleBased.config import (
    PASS_LIKE_OPEN, 
    SET_PIECE, 
    SIMPLE_TYPES, 
    SHOT_TYPES, 
...
def needleman_wunsch(
    ...
    if n_true == 0 and n_pred == 0:
        return []
    if n_true == 0:
        return [(None, j, "gap_true") for j in range(n_pred)] # gap_true: 예측 이벤트가 실제 이벤트와 매칭되지 않고 남는 경우
    if n_pred == 0:
        return [(i, None, "gap_pred") for i in range(n_true)] # gap_pred: 실제 이벤트가 예측 이벤트와 매칭되지 않고 남는 경우
...
def _pair_score(true_event: pd.Series, pred_event: pd.Series, scoring: dict | None = None, fps: int = 25) -> float:
    ...
    if "frame_id" in true_event and "frame_id" in pred_event and true_event["frame_id"] == pred_event["frame_id"]:
        ...
        td = abs(true_event["frame_id"] - pred_event["start_frame"]) / fps
        time_score = cfg["time_weight"] * (1.0 - min(td, max_td) / max_td)
        if td >= max_td:
            time_score = 0.0

    dist = ((true_event["x"] - pred_event["start_x"]) ** 2 + (true_event["y"] - pred_event["start_y"]) ** 2) ** 0.5
...
def add_alignment_details(
    ...
    result["_pred_frame_id"] = result["pred_index"].map(pred_events["start_frame"])
    ...
    result["_pred_event_type"] = result["pred_index"].map(pred_events["type"])
...
def evaluate_time_location_grid(
    ...
    if visualize:
        ...
        for title, df in zip(titles, dfs):
            ...
            ax.set_title(f"<{title}>\n", fontdict={"size": 12 * font_scale, "weight": "bold"})
...
def align_true_and_pred(
    ...
    pred_events = pred_events[~pred_events["subtype"].isin(["carry", "pause"])]
    out_node_dict = {
        "OUT-B": "out_bottom",
        "OUT-T": "out_top",
        "OUT-R": "out_right",
    ...
    pred_events["player_id"] = pred_events["player"].str.replace("H", "home_").str.replace("A", "away_")
    pred_events["player_id"] = pred_events["player_id"].replace(out_node_dict)  

    pred_events["receiver_id"] = pred_events["receiver"].str.replace("H", "home_").str.replace("A", "away_")
    pred_events["receiver_id"] = pred_events["receiver_id"].replace(out_node_dict)
    ...
    pred_counts = pred_events["type"].value_counts()
    ...
    alignments = []
    episode_ids = np.sort(pd.Index(true_events["episode_id"]).union(pred_events["episode"]).unique())
    for episode_id in tqdm(episode_ids):
        ep_true = true_events[true_events["episode_id"] == episode_id]
        ep_pred = pred_events[pred_events["episode"] == episode_id]
        ...
        for true_idx, pred_idx, move in ep_alignment:
            ...
            if (
                ...
                and true_event["event_type"] == pred_event["type"]
                # and abs(true_event["seconds"] - pred_event["seconds"]) <= threshold
                and abs(true_event["frame_id"] - pred_event["start_frame"]) / fps <= threshold
            ):

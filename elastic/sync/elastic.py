...
class ELASTIC:
    ...
    def __init__(
        ...
        if detect_controls:
            self.events = ELASTIC.insert_control_events(self.events)
    ...
    def find_event_episodes(self, events: pd.DataFrame) -> pd.DataFrame:
        ...
        for period_id in events["period_id"].dropna().unique():
            ...
            period_events_sorted["utc_timestamp"] = period_events_sorted["utc_timestamp"].astype("datetime64[ns]")

            period_frames = self.frames[self.frames["period_id"] == period_id].sort_values("utc_timestamp").copy()
            period_frames["utc_timestamp"] = period_frames["utc_timestamp"].astype("datetime64[ns]")
    ...
    def run(self, events: pd.DataFrame) -> pd.DataFrame:
        ...
        # TODO: events가 None이거나 episode_id 컬럼이 없는 경우에만 find_event_episodes를 호출하도록 변경
        # control 안 찍혀서 두 번 실행 방지
        if events is None or "episode_id" not in events.columns:
            print("Finding event episodes...")
            events = self.events.copy()
        ...
        if len(matches) > 0:
            ...
            aligned = aligned.loc[aligned.index.isin(events.index)].copy()
...
def run_match(match_id: str, in_event_dir: str, in_tracking_dir: str, out_event_dir: str, fps: float) -> pd.DataFrame:
    event_path = f"{in_event_dir}/{match_id}.parquet"
    tracking_path = f"{in_tracking_dir}/{match_id}.parquet"

    if not os.path.exists(event_path):
        raise FileNotFoundError(f"Event input file not found: {event_path}")
    if not os.path.exists(tracking_path):
        raise FileNotFoundError(f"Tracking input file not found: {tracking_path}")

    events = pd.read_parquet(event_path)
    tracking = pd.read_parquet(tracking_path)

    # Normalize parquet-loaded datetimes so merge_asof sees matching dtypes.
    if "utc_timestamp" in events.columns:
        events["utc_timestamp"] = pd.to_datetime(events["utc_timestamp"])
    if "utc_timestamp" in tracking.columns:
        tracking["utc_timestamp"] = pd.to_datetime(tracking["utc_timestamp"])

    syncer = ELASTIC(events, tracking, fps=fps)
    synced_events = syncer.run(events)

    os.makedirs(out_event_dir, exist_ok=True)
    output_path = f"{out_event_dir}/{match_id}.parquet"
    synced_events.to_parquet(output_path)
    return synced_events


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run ELASTIC from parquet inputs.")
    parser.add_argument("--match_id", type=str, default=None, help="Single match id to process, e.g. J03WMX")
    parser.add_argument("--in_event_dir", type=str, default="data/sportec/event_input")
    parser.add_argument("--in_tracking_dir", type=str, default="data/sportec/tracking_input")
    parser.add_argument("--out_event_dir", type=str, default="data/sportec/event_synced")
    ...
    if args.match_id is not None:
        ...
        match_ids = sorted(f.split(".")[0] for f in os.listdir(args.in_event_dir) if f.endswith(".parquet"))

    for i, match_id in enumerate(match_ids):
        print()
        print(f"[{i + 1}] {match_id}")
        synced_events = run_match(match_id, args.in_event_dir, args.in_tracking_dir, args.out_event_dir, args.fps)
        synced_count = synced_events["frame_id"].notna().sum()
        print(f"Saved {len(synced_events)} events to {args.out_event_dir}/{match_id}.parquet")
        print(f"Synced events with frame_id: {synced_count}")

...
from elastic.sync.config import PITCH_X, PITCH_Y
...
POSITION_MAPPING = {
    ...
    "OLM": "LAM",
    "ZO": "CAM",
    "OHR": "RAM",  # Offensiver Halbrechter
    "OHL": "LAM",  # Offensiver Halblinker
    "OHZ": "CAM",
    "DHR": "RDM",  # Defensiver Halbrechter
    "DHL": "LDM",  # Defensiver Halblinker
    "RA": "RWF",
    "LA": "LWF",
    "STR": "RCF",
    "STL": "LCF",
...
def _get_position(code):
    """Lookup position with fallback to UNKNOWN."""
...
class SportecData(MatchData):
    def __init__(self, data_dir: str, match_id: str, load_tracking: bool = True):
        super().__init__()
        self.match_id = match_id
        match_path = os.path.join(data_dir, match_id)
                    ...
                    lineup_list.append(
                        {
                            ...
                            "playing_position": _get_position(player.attrib.get("PlayingPosition")),
    ...
    def find_spadl_event_types(events: pd.DataFrame) -> pd.DataFrame:
        ...
        # KickOff is also represented as <Pass> inside <KickOff> wrapper.
        # Without this line the kickoff label is lost (becomes plain 'pass').
        events.loc[pass_mask & (events["set_piece_type"] == "KickOff"), "spadl_type"] = "kickoff"
        ...
        # Long throw-in (ThrowIn wrapper with Cross child) — keep as throw_in,
        # not cross. SPADL has no `throw_in_crossed`, so we collapse into throw_in.
        events.loc[cross_mask & (events["set_piece_type"] == "ThrowIn"), "spadl_type"] = "throw_in"
